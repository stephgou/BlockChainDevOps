...
if [ "${START_WITH_MINING}" = true ] ; then
	geth --maxpeers 10 --rpc --rpcaddr "${ETHEREUM_NODE_IP}" --rpcport "8545" --rpccorsdomain "*" --rpcvhosts "*" --datadir "${BLOCKCHAIN_DIR}" --rpcapi "db,eth,net,web3" --nat none --syncmode=fast --networkid "${ETHEREUM_NETWORK_ID}" --identity "${ETHEREUM_NODE_IDENTITY=}" --unlock "${ETHEREUM_ACCOUNT_ADDRESS}" --password "${ETHEREUM_ACCOUNT_PWD_FILE}" --nodiscover --mine
else
	geth --maxpeers 10 --rpc --rpcaddr "${ETHEREUM_NODE_IP}" --rpcport "8545" --rpccorsdomain "*" --rpcvhosts "*" --datadir "${BLOCKCHAIN_DIR}" --rpcapi "db,eth,net,web3" --nat none --syncmode=fast --networkid "${ETHEREUM_NETWORK_ID}" --identity "${ETHEREUM_NODE_IDENTITY=}" --unlock "${ETHEREUM_ACCOUNT_ADDRESS}" --password "${ETHEREUM_ACCOUNT_PWD_FILE}" --nodiscover
fi
